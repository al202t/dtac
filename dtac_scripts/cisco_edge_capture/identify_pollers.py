...
class ActionPollers():
	...
	def __call__(self):
		...
		self.print_message(f"[+] Running `ip check` on provided devices to identify ip addresses")
	...
	def iterrate_over_devices(self):
		...
		for device in self.devices:
			...
			self.devices_updated[device]['device'] = device
			self.devices_updated[device]['device_ip'] = self.collect_ip(self.devices_updated[device]['device'])
	...
	def collect_ip(self, device):
		...
		if not self.is_ip_pinging(device):  
	...
	def is_ip_pinging(self, device):
		if not device: return False
		self.write_channel(f"ping {device}\r\n")
		...
		for line in result_line.splitlines():
			...
			if self.device_ip is None and "bytes from" in line:
				self.device_ip = line.split("(")[-1].split(")")[0]
				continue
			#
			if not "packet loss" in line: continue
			if "100% packet loss" in line:
				self.print_message( f"[-] Device Unreachable: {device}, {self.device_ip}")
				return False
			return True
		self.print_message( f"[-] Device Unreachable: {device}, {self.device_ip}")
		return False
	...
	def dict_info(self):
		...
		for v in self.devices_updated.values():
			...
			if v['device_ip']:
				list_of_devices.append({'server': self.servers_list[i], 'device': v['device'], 'device_ip': v['device_ip'] })
				i+=1
